...
public class AdmissionDaoImpl implements IAdmissionDao {
	...
	public List<Student> getStudentsByCourse(String courseTitle) {
		...
		Transaction tx = session.beginTransaction();
		try {
			students = session.createQuery(jpql, Admission.class).setParameter("title", courseTitle).getResultStream() // Stream<Admission>
	...
	public List<Student> getStudentsByCourseOptimized(String courseTitle) {
		List<Student> students = null;
		// String jpql="select a from Admission a join fetch a.student where
		// a.course.title=:title";
		// OR
